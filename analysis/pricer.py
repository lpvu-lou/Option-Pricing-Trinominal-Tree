...
def display_trees(wb, tree, show_stock, show_reach, show_option, threshold=1e-7):
    ...
    def vertical_tree(levels, attr, decimals=6, hide_small=False):
        """
        ...
        for i, level in enumerate(levels):
            ...
            for j, node in enumerate(level):
                ...
                if value is None:
                    continue
                if hide_small and abs(value) < threshold:
                    val = ""
                else:
...
def run_pricer():
    ...
    # Nettoyage de la feuille de frontière
    ...
    # Si option américaine → afficher la frontière
